...
Развиваем класс `SecureMessage` из Task 1, добавляя интуитивно понятный интерфейс работы через операторы (тема "Перегрузка операторов"). Все изменения нужно внести в файлы из Task 1 (`secure_message.hpp` и `secure_message.cpp`).

### Требования:

- Подключить заголовок `<stdexcept>` для исключений и `<iostream>` для потоков вывода.
- Написать **оператор сравнения** `bool operator==(const SecureMessage& other) const`.
  - Должен сравнивать содержимое строк, а не указатели (сначала сравнить длины `length_`, затем посимвольно в цикле).
  - *Особенность C++20/23:* `operator!=` реализовывать не нужно, компилятор сгенерирует его автоматически на основе `==`.
- Написать **оператор конкатенации** `SecureMessage operator+(const SecureMessage& other) const`.
  - Возвращает новый объект, содержащий склейку двух сообщений.
  - Оптимизация: память под новую строку выделяется ровно один раз (размером `len1 + len2 + 1`).
- Написать **оператор добавления** `SecureMessage& operator+=(const SecureMessage& other)`.
  - Дописывает содержимое `other` в конец текущего объекта.
  - Сохранять строгую гарантию исключений: сначала выделяем новый буфер, копируем туда старую строку, затем новую, и только в конце (если не было исключений) освобождаем старую память и обновляем указатель.
- Написать **операторы индексации** `char& operator[](size_t index)` и `const char& operator[](size_t index) const`.
  - В случае выхода за пределы строки (`index >= length_`) должны бросать исключение: `throw std::out_of_range("Index out of range");`.
- Написать дружественную функцию **оператора вывода** `friend std::ostream& operator<<(std::ostream& os, const SecureMessage& msg)`.
  - Позволяет выводить объект в поток (`std::cout << msg`).
  - Выводит `data_` (если `nullptr`, можно ничего не выводить или выводить пустую строку). Обрати внимание, что эта функция объявляется внутри класса, но реализуется как свободная функция (вне `namespace context7` или внутри него, но без `SecureMessage::`).
