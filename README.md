...
- Написать **конструктор от `const char*`**, выделяющий память и копирующий переданную C-строку.
...
- Проверять результаты всех вызовов функций выделения памяти на `NULL`.
- По завершении работы вся динамически выделенная память должна быть освобождена.
